...
- Bindings del Route Worker histórico `rutas-campings-areas` verificados en Cloudflare: `DB` → `rutas-campings-areas-dev` y `PREMIUM_AUTH` → `campings-areas-premium`. Variables observadas: `OPENAI_MEDIA_ENABLED=false`, `OPENAI_ROUTE_PIPELINE_ENABLED=false`, `OPENAI_SPEND_ENABLED=false`; `OPENAI_TEST_ENDPOINT_ENABLED` no existe y el código la interpreta como `false`. `OPENAI_API_KEY` existe como secreto cifrado.
- Binding del Route Worker modular `rutas-campings-areas-nuevo` verificado: `PREMIUM_AUTH` → `campings-areas-premium`; no usa D1 histórico. `OPENAI_ROUTE_PIPELINE_ENABLED=true` y `OPENAI_SPEND_ENABLED=true` permanecen así deliberadamente para la prueba posterior ya prevista; no se modificaron. El frontend de producción no apunta a este Worker.
- El Premium Worker versionado reproduce el despliegue vigente y no contiene cambios funcionales pendientes; no se redesplegará innecesariamente. La migración D1 tampoco se ejecutará. Pendiente inmediato: desplegar únicamente las fuentes sincronizadas de ambos Route Workers, repetir health/CORS/cost guard gratuitos, publicar el frontend del PR y sólo entonces autorizar una única prueba real controlada.
