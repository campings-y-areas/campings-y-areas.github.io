...
function requestedPointIds(trip) {
  return new Set((trip.waypoints ?? []).map(point => String(point.request_point_id ?? point.id)));
}
...
export function buildVacationDays(trip) {
  ...
  stages.forEach((stage, stageIndex) => {
    ...
    days.push({
      ...
      logistics_id: `drive:${stage.driving_stage_id}`,
      day: driveDayNumber,
      ...
      driving_stage_index: stageIndex + 1,
      ...
      requested_waypoint: requestedPointIds(trip).has(String(stage.to_point_id)),
      is_final: stageIndex === stages.length - 1,
      stay_eligible: bases[stageIndex].eligible
    });
    ...
    for (let extra = 0; extra < bases[stageIndex].assigned; extra += 1) {
      ...
      days.push({
        ...
        logistics_id: `stay:${stage.driving_stage_id}:${extra + 1}`,
        day: stayDayNumber,
        ...
        base_id: stage.base_id ?? stage.overnight_id ?? null,
        overnight_id: stage.overnight_id ?? null,
        place: stage.to?.label ?? stage.overnight?.nombre ?? stage.overnight?.name ?? "Estancia",
        country: stage.to?.country ?? stage.overnight?.pais ?? "",
        driving_km: 0,
        driving_minutes: 0,
        request_point_id: stage.to?.request_point_id ?? null,
