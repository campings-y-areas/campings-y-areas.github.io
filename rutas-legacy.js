...
function consultasFotoVisita(nombre,ciudad,tipo="visit",terminosExtra=[]){
  ...
  if(tipo==="restaurant"){
    ...
    add(`"${original}" restaurant ${ciudadTxt}`);
    add(`"${original}" ${ciudadTxt} restaurant`);
    add(`"${original}" ${ciudadTxt}`);
    return q.slice(0,5);
  }
  if(tipo==="overnight"){
    (Array.isArray(terminosExtra)?terminosExtra:[]).forEach(add);
    add(`"${original}" camping ${ciudadTxt}`);
    add(`"${original}" campsite ${ciudadTxt}`);
    add(`"${original}" caravan ${ciudadTxt}`);
    add(`"${original}" ${ciudadTxt}`);
    return q.slice(0,6);
  }
...
function puntuacionFotoFinal(p,nombre,ciudad,tipo="visit"){
  ...
  if(tipo==="restaurant"){
    if(coincidenciasNombre===0)return -999;
    if(/restaurant|restaurante|brasserie|bistro|cafe|café|dining|facade|façade|interior|terrace|terrasse|cuisine/.test(texto))score+=22;
    if(/book|books|librairie|bookstall|bouquiniste|kiosk|kiosque|newsstand|shop|store|poster|archive|historic photo|historical photo|old photograph|carte postale|postcard/.test(texto))score-=80;
    if(/19[0-8][0-9]|18[0-9]{2}/.test(texto))score-=35;
  }

  if(tipo==="overnight"){
    if(coincidenciasNombre===0)return -999;
    if(/camping|campsite|campground|caravan|motorhome|rv|pitch|parcel|mobile home|mobilhome|aerial|pool|swimming|reception|rezeption/.test(texto))score+=25;
    if(/aerial|overview|panoram|pitch|parcel|caravan|motorhome|mobile home|pool|swimming/.test(texto))score+=18;
    if(/tunnel|underpass|passage|subway|bridge underside|road|street|entrance only|gate|barrier|sign|logo|map/.test(texto))score-=90;
  }
  return score;
}
async function buscarFotoAutomatica(nombre,ciudad,tipo="visit",terminosExtra=[]){
  ...
  try{
    ...
    completas.sort((a,b)=>puntuacionFotoFinal(b,nombre,ciudad,tipo)-puntuacionFotoFinal(a,nombre,ciudad,tipo));
    const ganadora=completas[0]; if(!ganadora)return null;
    const score=puntuacionFotoFinal(ganadora,nombre,ciudad,tipo);
    const minimo=tipo==="visit"?36:(tipo==="restaurant"?70:78);
...
async function consultarPlanificadorIA(datos,lugares,stops,vacationDays=[]){
  return llamarWorker("/plan-route",perfilWorker(datos,lugares,stops,vacationDays));
}

async function consultarRedactorIA(datos,lugares,stops,plan,vacationDays=[]){
  const perfil=perfilWorker(datos,lugares,stops,vacationDays);
...
function restaurantesParaDiaPremium(research,visitas,indiceDia,usadosGlobal=null){
  const lista=(Array.isArray(research?.gastronomy?.restaurants)?research.gastronomy.restaurants:[]).filter(x=>x?.name);
  if(!lista.length)return [];
  const usados=usadosGlobal instanceof Set?usadosGlobal:new Set();
  const porDia=Math.min(2,lista.length);
  const inicio=(indiceDia*porDia)%lista.length;
  const out=[];
  for(let n=0;n<lista.length&&out.length<porDia;n++){
    const x=lista[(inicio+n)%lista.length];
    const clave=normalizarClaveMedia(x.name);
    if(!clave||usados.has(clave))continue;
    if(!out.some(y=>normalizarClaveMedia(y.name)===clave)){
      out.push(x);
      usados.add(clave);
    }
  }
...
function htmlPortadaRuta(datos={}){
  ...
  const dias=Math.max(1,Number(datos.dias)||1);
  const jornadas=Math.max(1,Number(datos._jornadasConduccion)||1);
  const maxHoras=Math.max(1,Number(datos.maxConduccion)||4);
  const vehiculo=({autocaravana:"Autocaravana",camper:"Camper",caravana:"Caravana",coche:"Coche",moto:"Moto"})[datos.vehiculo]||"Viaje";
  const tarjetasPortada=demoEnCurso
    ...
    : `<div><div style="font-size:34px">📅</div><strong>${dias} ${dias===1?"día":"días"}</strong><small style="display:block;letter-spacing:.12em">VACACIONES</small></div>
      <div><div style="font-size:34px">🛣️</div><strong>${jornadas}</strong><small style="display:block;letter-spacing:.12em">JORNADAS DE CONDUCCIÓN</small></div>
      <div><div style="font-size:34px">🚐</div><strong>${escapar(vehiculo)}</strong><small style="display:block;letter-spacing:.12em">VEHÍCULO</small></div>
      <div><div style="font-size:34px">⏱️</div><strong>≤ ${maxHoras} h/día</strong><small style="display:block;letter-spacing:.12em">LÍMITE ELEGIDO</small></div>`;
...
async function calcularRuta(lugares,datos){
  ...
  const params=new URLSearchParams({waypoints,mode:modoGeoapify(datos.vehiculo),intermediate_waypoint_mode:"stopover",traffic:"approximated",units:"metric",lang:"es",format:"geojson",apiKey:config.GEOAPIFY_API_KEY});
...
async function pintarResultado(data,lugares,datos){
  ...
  pintarMetricas(distancia,tiempo,diasSolicitados,jornadasConduccion,lugares.length);
  ...
  if(conParadas.length){
    try{
      ...
      pintarMetricas(pOpt.distance||0,pOpt.time||0,diasSolicitados,jornadasConduccion,lugaresOpt.length);
...
function pintarMetricas(distancia,tiempo,diasVacaciones,jornadasConduccion,puntos){
  document.getElementById("metricasRuta").innerHTML=`
    ...
    <div class="metrica-ruta">📅 Vacaciones<strong>${Math.max(1,Number(diasVacaciones)||1)} días</strong></div>
    <div class="metrica-ruta">🛣️ Jornadas de conducción<strong>${Math.max(1,Number(jornadasConduccion)||1)}</strong></div>
...
const coordPernocta=[Number(cand.lon),Number(cand.lat)];
...
const paisPernocta=paisCanonico(
  revPernocta?.country_code||cand._countryCode,
  revPernocta?.country||cand.pais
);

return {
  minutes:e.idaMin,
  km:Number(e.ida.distance||0)/1000,
  place:localidadPernocta,
  country:paisPernocta,
...
async function prepararEsqueletoVacaciones(stops,datos){
  const etapas=Array.isArray(stops)?stops.map(x=>({...x})):[];
  const totalDias=Math.max(1,Number(datos?.dias)||1);
  ...
  if(days.length!==totalDias)throw new Error("No se pudo construir exactamente la duración de vacaciones solicitada.");
  return days;
}
...
formRuta.addEventListener("submit",async event=>{
  ...
  try{
    ...
    if(esDemo){
      ...
      // FASE42: la DEMO es un contenido editorial fijo y permanente.
      // No depende de la clave dinámica de ai_route_cache ni de researched_at.
      // Así, actualizar investigación o multimedia en D1 nunca puede romper la demo.
      // Este contenido es exactamente la guía de Saarlouis → Günzburg → Salzburg → Zagreb
      // que ya fue generada y aprobada anteriormente; no realiza llamadas a OpenAI.
      ...
      datos._jornadasConduccion=stops.length;
    ...
    // La logística diaria se cierra ANTES de consultar IA. El Planificador puede
    ...
    try{
      ...
      if(planCache?.ok&&planCache?.status==="research_required"&&Array.isArray(planCache?.missing_research)&&planCache.missing_research.length){
        ...
        if(investigacionPlan?.ok){
          ...
          planCache=await consultarPlanificadorIA(datos,lugares,stopsCache,vacationDays);
          estadoPlanIA=planCache;
        }
      }
      ...
      if(planCache?.ok&&planCache?.status==="planned"&&planCache?.plan){
        ...
        let guiaCache=await consultarRedactorIA(datos,lugares,stopsCache,planCache.plan,vacationDays);
        ...
        if(guiaCache?.ok&&guiaCache?.status==="research_required"&&Array.isArray(guiaCache?.missing_research)&&guiaCache.missing_research.length){
          ...
          if(investigacionGuia?.ok){
            ...
            guiaCache=await consultarRedactorIA(datos,lugares,stopsCache,planCache.plan,vacationDays);
          }
        }
        ...
        if(["research_required","media_research_required","cost_guard_active","invalid_route_contract","invalid_verified_content"].includes(guiaCache?.status)){
          pintarResultadoBase(ruta,lugares,datos);
          montarPortadaAntesMapa(datos);
          colocarResumenDebajoMapa();
          instalarAccionesRuta(lugares,stopsCache,datos);
          ...
          document.getElementById("estadoCalculo").textContent=
            guiaCache.status==="research_required"
              ...
              : guiaCache.status==="media_research_required"
                ...
                : (guiaCache.status==="invalid_route_contract"||guiaCache.status==="invalid_verified_content") ? "Ruta bloqueada por validación" : "Ruta pendiente de planificación IA";
          return;
        }
      }
      ...
      if(["research_required","media_research_required","cost_guard_active","invalid_route_contract","invalid_verified_content"].includes(planCache?.status)){
        pintarResultadoBase(ruta,lugares,datos);
        montarPortadaAntesMapa(datos);
        colocarResumenDebajoMapa();
        instalarAccionesRuta(lugares,stopsCache,datos);
        ...
        document.getElementById("estadoCalculo").textContent=
          planCache.status==="research_required"
            ...
            : planCache.status==="media_research_required"
              ? "Ruta pendiente de fotografías verificadas"
              : (planCache.status==="invalid_route_contract"||planCache.status==="invalid_verified_content") ? "Ruta bloqueada por validación" : "Ruta pendiente de planificación IA";
        return;
      }
    }catch(errCache){
      console.error("No se pudo completar el flujo Premium de la ruta.",errCache);
      pintarResultadoBase(ruta,lugares,datos);
      montarPortadaAntesMapa(datos);
      colocarResumenDebajoMapa();
      instalarAccionesRuta(lugares,stopsCache,datos);
      document.getElementById("etapasRuta").innerHTML=`<div class="error-ruta"><strong>⚠️ No se ha creado una guía incompleta.</strong><br>${escapar(errCache?.message||"Se produjo un error al validar la ruta.")}<br>La carretera y las jornadas calculadas se conservan, pero la guía Premium se detiene hasta poder validarse correctamente.</div>${htmlEtapasGenericas(stopsCache)}`;
      document.getElementById("estadoCalculo").textContent="Ruta detenida por seguridad de datos";
      return;
    }

    // Ningún estado desconocido puede degradarse a una falsa guía basada solo en
    // el destino final. Si el Worker no devuelve una ruta validada, detenemos aquí.
    pintarResultadoBase(ruta,lugares,datos);
    montarPortadaAntesMapa(datos);
    colocarResumenDebajoMapa();
    instalarAccionesRuta(lugares,stopsCache,datos);
    document.getElementById("etapasRuta").innerHTML=`<div class="aviso-ruta"><strong>⚠️ La ruta no ha superado la validación completa.</strong><br>No se ha generado contenido de sustitución. Reintenta cuando el servicio de planificación esté disponible.</div>${htmlEtapasGenericas(stopsCache)}`;
    document.getElementById("estadoCalculo").textContent="Ruta pendiente de validación";
    return;
  }catch(e){
