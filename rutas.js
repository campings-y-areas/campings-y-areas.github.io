...
function htmlPortadaRuta(datos={}){
  const origen=String(datos.origen||"Origen").trim();
  const destino=String(datos.destinoPrincipal||"Destino").trim();
  const titulo=`${escapar(origen)} → ${escapar(destino)}`;
  return `<section class="portada-ruta-fija" style="
    ...
    margin:0 0 28px;
    background-image:
      linear-gradient(180deg,rgba(255,255,255,.18) 0%,rgba(255,255,255,.05) 30%,rgba(2,27,47,.18) 68%,rgba(2,27,47,.66) 100%),
      url('${PORTADA_FONDO_DATA_URL}');
  ...
  ">
    <div style="
      ...
      flex:0 0 auto;
    ">
      <img src="logo.png" alt="Campings & Áreas" style="
        display:block;
        width:100%;
        height:100%;
        object-fit:contain;
    ...
    ">CAMPINGS & ÁREAS · VIAJAR · DESCUBRIR · DISFRUTAR</div>
  </section>`;
}
